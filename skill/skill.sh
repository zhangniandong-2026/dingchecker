...
cmd_collect() {
    ...
    echo ""
    print_info "🤖 [CLAUDE-ANALYZE] 请生成分析报告："
    echo ""
    echo "【报告位置】"
    echo "  $VAULT_OUTPUT/report_${date_arg}.md"
    echo ""
    echo "【报告内容】"
    echo "  1️⃣ 预警信息（三类区分）"
    ...
    echo "  2️⃣ 综合排名表格（横向比较，包含战队/战区分组）"
    echo "  3️⃣ 详细评分（每个单元展开，标注优秀/良好/及格/需改进）"
    echo "  4️⃣ 改进建议（针对性指导）"
    echo ""
    echo "【评分维度（总分20分）】"
    echo "  1. 昨日战果 (1-5分): 客户名+动作+结果+数据"
    echo "  2. 今日计划 (1-5分): 客户名+动作+deadline"
    echo "  3. 协同效率 (1-5分): 管理者响应+协同方反馈"
    echo "  4. 点评效率 (1-5分): 发言时长占比20-30%+实质指导"
    echo ""
    echo "【评分标准】"
    echo "  18-20分: 优秀 | 15-17分: 良好 | 12-14分: 及格"
    echo "  9-11分: 需改进 | <9分: 不合格"
    echo ""
    echo "【格式要求】"
    echo "  - 紧凑风格，评分标准用双链 [[meeting_guidelines_v4]] 引用"
    echo "  - 包含 frontmatter (title/created/tags/type/summary)"
    echo "  - 标记 #ai-generated #dingtalk #daily-companion"
    echo ""
    echo "参考: $PROJECT_ROOT/docs/meeting_guidelines_v4.md"
    echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
}
